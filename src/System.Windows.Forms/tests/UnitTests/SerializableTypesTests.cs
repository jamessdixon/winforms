...
namespace System.Windows.Forms.Tests.Serialization
{
    ...
    {
        [Fact]
        public void AxHostState_RoundTrip()
        {
            string coreBlob;
            ...
            var result = BinarySerialization.EnsureDeserialize(coreBlob) as AxHost.State;
        ...
        {
            using (var cursor = new Cursor(Path.Combine("bitmaps", "hand.cur")))
            {
                var coreBlob = BinarySerialization.ToBase64String(cursor);

                Assert.Equal(ClassicCursor, coreBlob);

                var result = BinarySerialization.EnsureDeserialize(coreBlob) as Cursor;
                Assert.Equal(cursor.Size, result.Size);
                Assert.Equal(cursor.Tag, result.Tag);
                Assert.Equal(cursor.HotSpot, result.HotSpot);
                Assert.NotEqual(IntPtr.Zero, result.Handle);
            }
        }

        [Fact]
        public void ImageListStreamer_RoundTrip()
        {
            string coreBlob;
            ...
            // Assert.Equal(ClassicImageListStreamer, coreBlob);
        ...
        {
            var result = BinarySerialization.EnsureDeserialize(blob) as ImageListStreamer;
        ...
        [Fact]
        public void LinkArea_RoundTrip()
        {
            ...
            var result = (LinkArea)BinarySerialization.EnsureDeserialize(coreBlob);
            Assert.Equal(linkArea, result);
        }

        [Fact]
        public void ListViewGroup_RoundTrip()
        {
            ...
            // Assert.Equal(ClassicListViewGroup, coreBlob);
        ...
        {
            var result = BinarySerialization.EnsureDeserialize(blob) as ListViewGroup;
        ...
        [Fact]
        public void ListViewItem_RoundTrip()
        {
            string coreBlob;
            ...
            var result = BinarySerialization.EnsureDeserialize(coreBlob) as ListViewItem;
        ...
        [Fact]
        public void ListViewSubItemANdSubItemStyle_RoundTrip()
        {
            string coreBlob;
            ...
            var result = BinarySerialization.EnsureDeserialize(coreBlob) as ListViewSubItem;
        ...
        public void Padding_RoundTrip()
        {
            ...
            var result = (Padding)BinarySerialization.EnsureDeserialize(coreBlob);
            Assert.Equal(padding, result);
        }

        [Fact]
        public void ResXDataNode_RoundTrip()
        {
            var dataNode = new ResXDataNode("node1", "value1");
            var coreBlob = BinarySerialization.ToBase64String(dataNode);

            Assert.Equal(ClassicResXDataNode, coreBlob);

            var result = BinarySerialization.EnsureDeserialize(coreBlob) as ResXDataNode;
            Assert.Equal("node1", result.Name);
            DataNodeInfo nodeInfo = result.GetDataNodeInfo();
            Assert.Equal("value1", nodeInfo.ValueData);
            Assert.Empty(nodeInfo.Comment);
            Assert.Null(nodeInfo.MimeType);
            Assert.Null(nodeInfo.TypeName);
        }

        [Fact(Skip = "Encoding is not serializable")]
        public void ResXFileRef_Encoding_RoundTrip()
        {
            var encoding = Encoding.UTF8;//     new UTF8Encoding();
            var typeName = typeof(string).FullName;
            var fileRef = new ResXFileRef("fileName", typeName, encoding);
            var coreBlob = BinarySerialization.ToBase64String(fileRef);

            Assert.Equal(ClassicResXFileRef_Encoding, coreBlob);

            var result = BinarySerialization.EnsureDeserialize(ClassicResXFileRef_Encoding) as ResXFileRef;
            Assert.Equal("fileName", result.FileName);
            Assert.Equal(encoding, result.TextFileEncoding);
        ...
        public void TableLayoutSettings_RoundTrip()
        {
            ...
            var result = BinarySerialization.EnsureDeserialize(coreBlob) as TableLayoutSettings;
        ...
        public void TreeNodeAndPropertyBag_RoundTrip()
        {
            ...
            var result = BinarySerialization.EnsureDeserialize(coreBlob) as TreeNode;
        ...
        private const string ClassicCursor =
            "";
        ...
        private const string ClassicResXDataNode =
            "";

        private const string ClassicResXFileRef_Encoding =
            "";
